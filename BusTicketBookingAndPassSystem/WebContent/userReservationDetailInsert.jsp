		...
		<div class="gujju-row">
			<div class="gujju-col m4">
				<form name="reservation" method="post">
					...
					<font color=red size=5>${msgResDet}</font>
					<div class="btn text-primary">
						<font size="+3" style="text-align: center;">Select Seat:</font>
					</div>
					<div id="holder" style="margin-left: 260px;">
						<!-- Seat outer Div-->
						<ul id="place">
							<!-- Seat layout -->
						</ul>
					...
					<!-- --------------------------- submit button ----------------------------------------- -->
					<div
						style="margin-left: 900px; position: absolute; margin-top: -410px;">
						<button style="font-size: 30px;" type="button" id="btnShowNew"
							class="fa fa-check gujju-btn gujju-blue">&emsp;Confirm</button>
					</div>
					...
					<div class="container" style="margin-left: 260px;">
						<div class="row">
							...
							<div class="col-md-3">
								<img src="DigitalGSRTC-photos/Selected.gif"><b>Selected
									Seat</b>
							</div>
						</div>
					</div>
				...
				ArrayList <ReservationDetailBean> listOfReservationdetailBean= new ReservationDetailDAO().getReservationSeats(reservationBean);%>
					...
					<%for(int i=0;i<listOfReservationdetailBean.size();i++)
						...
						bookedSeats
									.push(
					<%=listOfReservationdetailBean.get(i).getSeatNum()%>
						);
					<%}%>
						init(bookedSeats);
							$('.' + settings.seatCss)
							...
							$('#btnShowNew')
									.click(
											function() {
												...
												alert(str.join(','));
												document.reservation.action = "UserReseravationDetailInsertServlet?seatId="
														+ str;
