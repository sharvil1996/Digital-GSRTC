...
protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	...
	{	
		...
		if(item.isFormField())
		{
			...
			fieldValue=item.getString();
				...
				else if(fieldName.equals("seldestination"))
				{	
	...
	{
		isError=true;
		request.setAttribute("destinationName", "<font color=red>* DestinationName is Required.......</font>");
	}
	else
	...
	if(isError)
	{
		...
		request.getRequestDispatcher("userPassDetailInsert.jsp").forward(request, response);
	}
	else
	{
		...
		if(new PassDAO().insert(passBean) )
		{
			...
			PassBean passBean2 = new PassDAO().getByUserId(passBean.getUserId());
			request.setAttribute("passBean", passBean2);
			request.getRequestDispatcher("userPassList.jsp").forward(request, response);
		}
